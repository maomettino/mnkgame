...
public class AlphaBetaPruning {
	...
	private Node inefficiente(Node father) {
		...
		for (Node c : children) {
			...
			if((child.isSaddam &&child.value==WIN) || (!child.isSaddam &&child.value== DEFEAT)) {
				System.out.println("since this move is winning for the current player we look no further in the current sub-tree");
				break;
			}
		}
	 ...
	 * }
	 * // System.out.println("depth " + currentDepth);
	 * // System.out.println("il valore del figlio pre-confronto max " +
	 * child.value);
	 * // System.out.println("il valore del padre pre-confronto max " +
	 * father.value);
	 * father.value = Math.max(father.value, child.value);
	 * father.alpha = Math.max(father.alpha, father.value);
	 * // System.out.println("il valore del padre post-confronto max " +
	 * father.value);
	 * } else {
	 * // System.out.println("depth " + currentDepth);
	 * // System.out.println("il valore del figlio pre-confronto min " +
	 * child.value);
	 * // System.out.println("il valore del padre pre-confronto min " +
	 * father.value);
	 * father.value = Math.min(father.value, child.value);
	 * father.beta = Math.min(father.beta, father.value);
	 * // System.out.println("il valore del padre post-confronto min " +
	 * father.value);
	 * }
	 * if (father.beta <= father.alpha) {
	 * System.out.println("cut off");
	 ...
	 * // System.out.println("checking free back H cell " + i + " " + (j -
	 * backCount));
	 * if (length == k - 1) {
	 ...
	 * // System.out.println("checking free V back cell " + (i - backCount) + " " +
	 * j);
	 * if (length == k - 1) {
	 ...
	 * // System.out.println("checking free D back cell " + (i - backCount) + " " +
	 * (j
	 * // - backCount));
	 * if (length == k - 1) {
	 ...
	 * // System.out.println("checking free back AD cell " + (i - backCount) + " " +
	 * (j
	 * // + backCount));
	 * if (length == k - 1) {
	 ...
	 * && b[i + forwardCount][j - forwardCount] == MNKCellState.FREE) {
	 * // System.out.println("checking forward AD cell " + (i + forwardCount) +" " +
	 * (j
	 * // - forwardCount));
	 * if (length == k - 1) {
	 ...
	 * if (isSaddam) {
	 * // System.out.println("saddamHistory size: "+saddamHistory.size()+"
	 * saddamCount
	 * // "+saddamCount+ " saddamMoves "+ saddamMoves);
	 ...
	 * }
	 * 
	 * private Node pippo(Node father) {
	 * currentDepth++;
	 * // System.out.println("depth " + currentDepth);
	 * Set<MNKCell> saveState = new HashSet<MNKCell>();
	 * saveState.addAll(father.isSaddam?saddamAdjacentCells:foeAdjacentCells);
	 * updateAdjacentCells((father.isSaddam?saddamAdjacentCells:foeAdjacentCells),
	 * new MNKCell(father.i, father.j));
	 * (father.isSaddam?foeAdjacentCells:saddamAdjacentCells).remove(new
	 * MNKCell(father.i, father.j));
	 * Node[] children = getNodes(father);
	 * //may be optimized?
	 * if(children.length==0)
	 * return new Node(father.i, father.j, father.isSaddam, 0);
	 ...
	 * if (currentDepth != MAX_DEPTH - 1) {
	 * 
	 * if(isWinningCell(children[i].i, children[i].j,
	 * !children[i].isSaddam?saddam:foe)) {
	 * System.out.println("found winning cell i: "+children[i].i
	 * +" j: "+children[i].j);
	 * return new Node(children[i].i, children[i].j, !father.isSaddam,
	 * children[i].isSaddam?WIN:DEFEAT);
	 * }
	 * //check here if there are game-ending moves
	 * b[children[i].i][children[i].j] = children[i].isSaddam ? saddam : foe;
	 * child = pippo(children[i]);
	 * b[child.i][child.j] = MNKCellState.FREE;
	 * } else {
	 * if(isWinningCell(children[i].i, children[i].j,
	 * !children[i].isSaddam?saddam:foe)) {
	 * System.out.println("porcodio mossa vincente i: "+children[i].i+" j: "
	 * +children[i].j);
	 * return new Node(children[i].i, children[i].j, !father.isSaddam,
	 * children[i].isSaddam?WIN:DEFEAT);
	 * }
	 * child = getHeuristicLeaf(children[i].i, children[i].j, !father.isSaddam,
	 * foeValue);
	 * }
	 * 
	 * if (children[i].isSaddam) {
	 ...
	 * else
	 * foeAdjacentCells = saveState;
	 * //goBack((father.isSaddam?saddamAdjacentCells:foeAdjacentCells), father);
	 * currentDepth--;
	 * return father;
	 * }
	 */
	...
	/*
	 * private void goBack(Set<MNKCell> set, Node father) {
	 * MNKCell myCell = new MNKCell(father.i, father.j);
	 * bool remove;
	 * for (int[] d : direction) {
	 * remove = true;
	 * int i = myCell.i + d[0];
	 ...
	 * int i2 = myCell.i + d2[0];
	 * int j2 = myCell.j + d2[1];
	 * 
	 * // TODO deve controllare se è il player attuale e non P1
	 * if (i2 < n && j2 < n && i2 >= 0 && j2 >= 0 && b[i2][j2] == MNKCellState.P1 &&
	 * (myCell.i != i2 || myCell.j != j2)) {
	 * remove = false;
	 * break;
	 * }
	 * }
	 * if (remove == true)
	 * set.remove(new MNKCell(i, j));
	 * }
	 * // TODO deve controllare se è il player attuale e non P1
	 * else if (i < n && j < n && i >= 0 && j >= 0 && b[i][j] == MNKCellState.P1){
	 * set.add(myCell);
	 * }
	 * }
	 * for (int[] d : direction) {
	 ...
	 * int j = myCell.j + d[1];
	 * // TODO deve controllare se è il player attuale e non P1
	 * if (i < n && j < n && i >= 0 && j >= 0 && b[i][j] == MNKCellState.P1) {
	 * set.add(foeCell);
	 * }
	 * }
	 * }
	 ...
	 * // System.out.println("for father i: "+father.i +" j: "+father.j + "
	 * saddamJump
	 * // " + saddamJump + " foeJump "+ foeJump+" saddamHistory size "+
	 * // saddamHistory.size()+ " foeHistory size "+ foeHistory.size() );
	 * Node[] children;
	 ...
	 * }
	 * /*
	 * S System.out.println("children in the:");
	 * for (Node child : children) {
	 * System.out.println("i: " + child.i + " j: " + child.j);
	 * }
	 */
	...
	/*
	 * if (children.length == 0) {
	 * //System.out.println("the node ahamdinejad i: " + father.i + " j: " +
	 * father.j
	 * // + " has no adjacents as well, must take the other player adjacents");
	 * //currentDepth--;
	 * Set<MNKCell> set = getAdjacentCells(father.isSaddam);
	 * //currentDepth++;
	 * int i=0;
	 ...
	 * // System.out.println("the node ahamdinejad i: " + father.i + " j: " +
	 * father.j
	 * // + " has no children at all, this shouldn't happen");
	 * 
	 * }
	 * return children;
	 * }
	 ...
	 * // forwardCount));
	 * if (length == k - 1) {
	 * // System.out.println("winning H forward move " + i + " " + (j +
	 * forwardCount));
	 * int move[] = { i, j + forwardCount };
	 * moves.win = move;
	 * return;
	 ...
	 * forwardExtra++;
	 * if (length + forwardExtra == k - 1) {
	 * // System.out.println("winning forward H move " + i + " " + (j +
	 * forwardCount));
	 * int move[] = { i, j + forwardCount };
	 * moves.win = move;
	 * return;
	 ...
	 * if (freeBack[0] && length + backExtra == k - 2 && freeForward[0] &&
	 * freeExtraBack) {
	 * // System.out.println("found 2 win back H move " + i + " " + (j -
	 * backCount));
	 * int move[] = { i, j - backCount };
	 * moves.twoWin = move;
	 * return;
	 ...
	 * j);
	 * if (length == k - 1) {
	 * // System.out.println("found winning V back cell " + (i - backCount) + " " +
	 * j);
	 * int move[] = { i - backCount, j };
	 * moves.win = move;
	 * return;
	 ...
	 * // - backCount));
	 * if (length == k - 1) {
	 * // System.out.println("found winning D back cell " + (i - backCount) + " " +
	 * (j
	 * // - backCount));
	 * int move[] = { i - backCount, j - backCount };
	 * moves.win = move;
	 ...
	 * // + backCount));
	 * if (length == k - 1) {
	 * // System.out.println("found winning back AD cell " + (i - backCount) + " " +
	 * (j
	 * // + backCount));
	 * int move[] = { i - backCount, j + backCount };
	 * moves.win = move;
